...
export default function OrderDetails({
  ...
  const fetchCart = async () => {
    ...
    try {
      ...
      setCart((prevCart) =>
        prevCart.concat(orderProductModel as OrderProductModel)
      );
    } catch (error) {
  ...
  const printOrder = async () => {
    try {
      ...
      // For each order detail, update in the database the quantity of the product bought
      for (let i = 0; i < cart.length; i++) {
        ...
        const newQuantity = product.bought + cart[i].quantity;

        const { error: updateError } = await supabase
          .from("products")
          .update({ bought: newQuantity })
          .eq("id", cart[i].id);
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="relative bg-white p-4 rounded-lg w-80">
            ...
            {cart.map((item, index) => (
              <div
                key={index}
                className="flex justify-between mb-4 pb-2 border border-b-[#BDBDBD] border-x-transparent border-t-transparent"
              >
